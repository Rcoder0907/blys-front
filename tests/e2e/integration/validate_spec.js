describe('Validation tests', () => {
    it('It should visit the page', () => {
      cy.visit('http://localhost:3000')
    })

    it('It should display error message with code enter', () => {
        cy.visit('http://localhost:3000');
        cy.get('button[class=submit-button]').click();
        cy.get('div[class=errorSms]').should('contain', 'Verification Error');
      })

      it('It should display error message with invalid character enter', () => {
        cy.visit('http://localhost:3000');
        cy.get('.input-box')
            .eq(0)
            .type('d')
      ...
      it('It should display error message if last digit is 7', () => {
        cy.visit('http://localhost:3000');
